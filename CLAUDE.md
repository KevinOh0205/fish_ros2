...
| `auto_scenario_node` | Time-based trajectory generator for AUTO mode. A scenario is a **step table** (duration + roll/pitch/yaw/thrust per step); adding one means adding a table in `build_steps()`. Six exist (2026-09-03): `straight` (default — level, 20 %, 3 s), `dive`, `climb`, `turn` (yaw relative to the heading at entry), `porpoise`, `attitude` (thrust 0, bench servo check). Selected by the `scenario` parameter with `run_sec` / `throttle_pct` / `angle_deg`; **all are latched at AUTO entry**, so a param change applies on the next entry, not mid-run. Every scenario cuts thrust at `run_sec`. |
