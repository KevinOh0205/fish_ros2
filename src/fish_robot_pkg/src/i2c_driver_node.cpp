...
//            ├─ ch0 → 0x76 MS5837 (J5 커넥터)
//            ├─ ch1 → 0x76 MS5837 (J6 커넥터)
//            └─ ch2 → 0x76 MS5837 (J7 커넥터)
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    if (ioctl(i2c_fd_, I2C_SLAVE, MS5837_ADDR) >= 0) {
                        ...
                        if (D1[i] != 0xFFFFFF && D2[i] != 0xFFFFFF && D1[i] != 0 && D2[i] != 0) {
                            // ---- MS5837 데이터시트 1차 온도 보상 환산식 ----
                            // C[1]=SENS_T1, C[2]=OFF_T1, C[3]=TCS, C[4]=TCO, C[5]=T_REF, C[6]=TEMPSENS
                            uint16_t *C = prom_C_[i];
                            int32_t dT = D2[i] - ((uint32_t)C[5] * 256);              // 기준 온도와의 차
                            int32_t TEMP = 2000 + ((int64_t)dT * C[6]) / 8388608;     // 실제 온도 (0.01°C 단위)
                            int64_t OFF = ((int64_t)C[2] * 65536) + (((int64_t)C[4] * dT) / 128);    // 온도 보상된 오프셋
                            int64_t SENS = ((int64_t)C[1] * 32768) + (((int64_t)C[3] * dT) / 256);   // 온도 보상된 감도
                            int32_t P = ((D1[i] * SENS / 2097152) - OFF) / 8192;      // 압력 (0.1 mbar 단위)

                            float raw_pressure = P / 10.0f;   // 0.1mbar -> mbar
