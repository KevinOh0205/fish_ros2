                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    if (ioctl(i2c_fd_, I2C_SLAVE, MS5837_ADDR) >= 0) {
                        ...
                        if (D1[i] != 0xFFFFFF && D2[i] != 0xFFFFFF && D1[i] != 0 && D2[i] != 0) {
                            // ---- MS5837-02BA 데이터시트 온도 보상 환산식 (1차 + 아래 2차) ----
                            ...
                            // ---- 02BA 2차 온도 보상 (데이터시트 SECOND ORDER 블록) ----
                            // 1차 식은 20°C 아래에서 오차가 남고 추울수록 제곱으로 커진다.
                            // 20°C 위에서는 세 항이 전부 0이라 기존 동작과 완전히 같다 —
                            // 물속(<20°C)에서만 조용히 발동한다. 영점은 25°C 공기에서 잡으므로
                            // 이 보정이 없으면 찬물에서 영점 뺄셈이 안 맞아 수심이 cm급으로
                            // 밀린다(뺄셈은 "같은 조건"일 때만 상쇄된다).
                            // ※ 계수(11/2^34, 31/2^3, 63/2^5)는 02BA 전용이다 — 30BA는 다르다.
                            if (TEMP < 2000) {
                                int64_t d2 = (int64_t)(TEMP - 2000) * (TEMP - 2000);
                                int32_t Ti    = (int32_t)(11 * ((int64_t)dT * dT) / 17179869184LL); // 11*dT^2/2^34
                                int64_t OFFi  = 31 * d2 / 8;                                        // 31*(TEMP-2000)^2/2^3
                                int64_t SENSi = 63 * d2 / 32;                                       // 63*(TEMP-2000)^2/2^5
                                TEMP -= Ti;
                                OFF  -= OFFi;
                                SENS -= SENSi;
                            }
