...
private:
    ...
    void publish_rpm() {
        // [실측 2026-08-27] 1회전 = 4펄스. 손으로 정확히 5바퀴(바퀴별 4,5,4,4,4) + 10바퀴(44펄스)
        // 두 시험이 모두 4를 가리킨다 — 종전 "3클럭 = 1회전" 스펙 가정은 실물과 다르다.
        // 스펙으로도 확정: BL4261 은 8극(극쌍 4) 모터 → 홀 한 라인 상승 엣지 = 회전당 4개.
        // (펄스 수 * 10Hz * 60초) / 4 펄스 = 펄스 수 * 150
        //
        // 주의 1: 100ms 창에서 세므로 분해능이 펄스 1개 = 150 RPM으로 거칠다.
        //         저속 구간을 세밀하게 봐야 하면 창을 늘리거나 펄스 간격(주기) 측정 방식으로 바꿔야 한다.
        // 주의 2: 모터 배터리 없이 축을 돌리면 펄스의 약 2/3가 누락된다(실측 — 홀 신호 레벨 저하).
        //         엔코더 시험은 반드시 배터리 연결 상태로 할 것.
        int pulses = pulse_count_.load();
        pulse_count_ = 0;   // 읽고 즉시 리셋 (load와 store 사이에 들어온 펄스는 유실 — 100Hz 회전에서는 무시 가능)

        current_rpm_ = pulses * 150;
