...
2026-08-17부터 이 스크립트가 로봇의 기본 캘리브레이션이다 — btn2 롱프레스 /
/calibrate_mag 가 state_estimation_ekf_node 에서 이 스크립트를 서브프로세스로 실행한다.
(예전 min/max 중점 방식은 극값에 닿아야만 맞는 방식이라, 2026-08-06 실측에서
**무보정보다 나빴다**: 흩어짐 38.1% vs 22.1%. 폐기.) 이 도구는 모든 점을 쓰는
타원체 피팅이라 극값에 안 닿아도 중심을 푼다.
...
def write_params(cen, scale, out):
    ...
    print(f"  ✅ 저장 → {out}")
    print("     ※ btn2 길게로 실행된 경우 state_estimation_ekf_node 가 종료 코드를 보고 자동 재적용합니다")
    print("     ※ 터미널에서 수동 실행했다면 state_estimation_ekf_node 재시작 필요 (시작 시 1회만 읽음)")
